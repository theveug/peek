...
- [ ] **Low / defense-in-depth — `connect-src` allows any `ws:`/`wss:` host** — `server.js`'s CSP `connect-src 'self' ws: wss:` permits WebSocket connections to any host, so if script execution were ever achieved despite the strict `script-src`, exfiltration over WS stays open. The explicit schemes were added because not every browser lets `'self'` cover the scheme change to the signalling socket — worth testing whether modern browsers now cover same-origin `ws(s)` under `'self'` alone (they largely do), and if so tightening to just `'self'`. Not exploitable on its own; only matters as a second line behind an XSS that doesn't currently exist.
- [ ] **Cosmetic consistency — creator-token compare uses `===`, not `timingSafeEqual`** — `claimModerator`/`isCreatorTokenValid` compare 128-bit random tokens with `===` while passwords use `crypto.timingSafeEqual`. A timing side-channel on a full-entropy random token isn't a realistic recovery vector, so this is consistency/hygiene rather than a real finding — fold it in if that code is touched anyway.
