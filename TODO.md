...
- [ ] **Verify adaptive quality tier downgrade end-to-end** — the participant-cap/adaptive-quality work (see `CHANGELOG.md`) was verified for the cap/rejection path, but the 7+ participant quality-downgrade tiers in `PeerManager._resolveQuality()` weren't exercised live. Needs 7+ simultaneous tabs each starting a real screen/cam stream, reading `track.getSettings()` (or `getStats()`) to confirm resolution/fps actually drops at the 7-8/9-10/11-12 thresholds and restores when participants leave. Heavier to drive headlessly (no real display to capture in CI), so do it as a deliberate one-off check rather than routine verification.

## Security (open — from the 2026-07-05 Fable review; the patched items are in CHANGELOG.md)

- [ ] **Self-host `marked` + `DOMPurify` (and pin/subset the highlight.js pack), or add SRI** — `index.html` loads `marked` from jsdelivr *completely unpinned* (`/npm/marked/` = latest), and none of the ~50 CDN `<script>`s carry an `integrity`/`crossorigin` attribute. A compromised CDN or a malicious `marked`/`DOMPurify` release runs arbitrary JS in every room — and since DOMPurify (our XSS sanitizer) comes from the same CDN, it can't defend itself. Also a privacy leak (every visitor's IP hits jsdelivr), which cuts against the no-server-storage/blind-relay ethos. Fonts are already self-hosted; do the same for these, or at minimum pin exact versions + add SRI hashes. Good moment to subset the ~200-file highlight.js language pack down to the languages actually worth supporting. **Highest-value open security item — carried over across two review passes now.**
- [ ] **Set Express `trust proxy` if deployed behind a reverse proxy** — the HTTP `rateLimit()` and the per-IP WS join throttle both key on the socket's remote address. Behind nginx/Cloudflare without `app.set('trust proxy', ...)`, every visitor shares the proxy's IP, so one abuser rate-limits *everyone* (and the per-IP throttle becomes a shared bucket). Deploy-time config, not a code bug — flagged so it isn't missed.
