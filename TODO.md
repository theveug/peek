...
- [ ] **Rem/clamp() font-scaling conversion (branch: `rem-font-scaling`, started 2026-07-13)** — mechanical conversion of `tailwind.css`'s ~600 hardcoded px sizing values to rem, plus a fluid `clamp(16px, 14.59px + 0.376vw, 20px)` root font-size (16px at a 375px viewport, 20px at 1440px) and a user-adjustable `--font-scale` multiplier (Settings → Appearance → Text size slider, `FontScaleManager.js`, 0.85–1.3x, persisted `localStorage['fontScale']`). Deliberately excluded from conversion: border/outline widths, box-shadow, transform, `@media` breakpoint conditions (all should stay tied to real pixels, not the user's text-size preference), and the `border-radius: 999px`/`9999px` "fully round pill" idiom. Visually spot-checked at 100%/120% across lobby, settings, and room UI (chat/participants/composer) — all scaled cleanly, no overlap/clipping. Full `test:ui`/`test:offline`/`test:peer-block` suite green. **Remaining before merge**: broader manual pass across surfaces not yet screenshotted (mobile breakpoints, focus/grid view, file cards, polls, the lightbox added this same day), and a check of inline `style="...px..."` usage in JS/HTML (out of scope for this first pass — only `tailwind.css`'s own custom classes were converted; Tailwind's utility classes like `w-4`/`text-sm` were already rem-based via the framework's own default scale and needed no changes).
