...
- [ ] **Discord-parity candidates that fit the no-server-storage principle (2026-07-11 gap review)** — from a feature-gap comparison against Discord; message edit/delete and session-sticky kicks shipped the same day (see `CHANGELOG.md`). Still open, all session-scoped/client-side: **pinned messages** (session-lifetime, pairs with existing moderator tiers), **in-chat search** (client-side over the in-memory log), **custom avatars** (small image P2P-shared on join, like nicknames), **custom status text** (broadcast alongside the online/away/dnd status), **vendored static emoji picker for chat composing** (reactions already have one; Tenor-style GIFs stay out — external requests violate the offline rule). **Rejected: voice messages** — Peek rooms are live voice calls, so async voice clips have no real use case here (owner confirmed; heavy Discord user who never uses them).
