...
- [ ] **Focus-view pause/resume gap** — the watch/unwatch pause mechanism (see Completed) is only wired up for grid view. Switching to focus view doesn't pause non-focused peers that were never built into a grid cell, and doesn't re-evaluate watch state when the focused peer changes outside of `focusStream()`'s own call sites. Not a regression (defaults to "keep sending," same as pre-Phase-2 behavior) but leaves bandwidth on the table for anyone who lives in focus view. Would need `setViewMode`/`toggleViewMode` to treat "focused peer in focus mode" the same way `buildGrid()` treats grid tiles.
