...
- [ ] **Rem/clamp() font-scaling conversion (own branch, likely multi-session, decided 2026-07-13)** — convert `tailwind.css` (~4200 lines) to rem-based sizing throughout, driven by a `clamp(16px, ..., 20px)` root font-size (fluid between a 16px mobile floor and 20px desktop ceiling, same approach as Bricks builder) so the whole UI scales from one adjustable base instead of fixed px. Explicitly scoped to its own branch rather than mixed into incremental feature work, since it's a mechanical rewrite of the entire stylesheet with real visual-regression risk — every major surface (lobby, room, settings, chat) needs to be re-verified after. Not started yet.
